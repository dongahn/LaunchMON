...
AC_DEFUN([X_AC_TEST_RM], [
  AC_MSG_CHECKING([resource manager to test @<:@slurm ibm_spectrum bgqrm alps orte intel_hydra@:>@])
  AC_ARG_WITH([test-rm],
    AS_HELP_STRING(--with-test-rm@<:@=RM@:>@,specify a resource manager type to test @<:@slurm ibm_spectrum bgqrm alps orte intel_hydra@:>@ @<:@default=slurm on linux-x86 and linux-x86_64; alps on Cray; bgqrm on linux-power64; ibm_spectrum on linux-power64le@:>@),
    [with_rm=$withval],
    [with_rm="check"])
